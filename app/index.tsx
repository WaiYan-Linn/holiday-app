...
export default function HolidayList() {
  ...
  return (
    <GlassCard
      ...
      hero={isHero} // NEW
    >
      {isHero && <View style={styles.nextBadge}><Text style={styles.badgeText}>UPCOMING NEXT</Text></View>}

      <View style={styles.row}>
        <View style={styles.dateBox}>
          <Text style={styles.day}>{new Date(item.date.iso).getDate()}</Text>
          <Text style={styles.month}>
            {new Date(item.date.iso).toLocaleString('en-US', { month: 'short' }).toUpperCase()}
          </Text>
        </View>

        <View style={styles.contentBox}>
          <Text style={[styles.name, isHero && styles.heroName]}>{item.name}</Text>
      ...
      {isHero && (
        <Text style={styles.desc} numberOfLines={3}>
          {item.description || "Prepare for the festivities! This is a national public holiday in Myanmar."}
        </Text>
      )}
    </GlassCard>
  );
